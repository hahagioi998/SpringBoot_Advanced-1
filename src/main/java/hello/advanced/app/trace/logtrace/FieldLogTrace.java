...
public class FieldLogTrace implements LogTrace {
    ...
    private void syncTraceId(){
        if(traceIdHolder == null){
            traceIdHolder = new TraceId();              //처음일 경우에는 traceIdHolder가 비어있으니까 그냥 생으로 하나 만들어줍니다.
        }else{
            traceIdHolder = traceIdHolder.createNextId();//처음이 아닌 경우에는 이전 helloTrace 클래스에서 사용했던 id는 유지하되 level을 올려서
                                                         //리턴을 받는 메서드를 사용해줍니다.
